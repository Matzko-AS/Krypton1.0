...
const PerfilModal = ({ usuario, perfil, onClose, onUsuarioActualizado }) => {
  const [tab, setTab] = useState("usuario") // "usuario" | "password"
  ...
  const handleGuardarUsuario = async () => {
    ...
    if (error) {
      ...
      setNuevoUsuario("")
      setUsuarioDisponible(null)
      if (onUsuarioActualizado) onUsuarioActualizado(nuevoUsuario.toLowerCase())
    }
  ...
  const handleGuardarPassword = async () => {
    ...
    if (passActual === passNueva)
      return setErrorPass("La nueva contraseña debe ser diferente a la actual.")
      ...
      <div className="perfil-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="perfil-tabs">
          <button
            ...
            onClick={() => { setTab("usuario"); setErrorUsuario(""); setOkUsuario("") }}
          >
            <i className="ti ti-user" /> Cambiar usuario
          </button>
          <button
            className={`perfil-tab ${tab === "password" ? "active" : ""}`}
            onClick={() => { setTab("password"); setErrorPass(""); setOkPass("") }}
          >
                ...
                <div className="perfil-input-wrap">
                  ...
                  <span className="perfil-input-status">
                    ...
                    {!checkingUsuario && usuarioDisponible === true && (
                      <i className="ti ti-circle-check" style={{ color: "#22c55e" }} />
                    )}
                    {!checkingUsuario && usuarioDisponible === false && (
                      <i className="ti ti-circle-x" style={{ color: "#ef4444" }} />
                    )}
                  </span>
                </div>
              ...
              <div className="perfil-field">
                ...
                <div className="perfil-input-wrap">
                  ...
                  <button
                    ...
                    type="button"
                  >
                    <i className={`ti ${verActual ? "ti-eye-off" : "ti-eye"}`} />
                  </button>
                </div>
              </div>
                ...
                {passNueva.length > 0 && (
                  <div className="perfil-strength">
                    <div
                      ...
                      style={{
                        width: passNueva.length >= 10 ? "100%" : passNueva.length >= 6 ? "60%" : "30%",
                        background: passNueva.length >= 10 ? "#22c55e" : passNueva.length >= 6 ? "#f59e0b" : "#ef4444",
                      }}
                    />
                  </div>
                )}
              ...
              <div className="perfil-field">
                ...
                <div className="perfil-input-wrap">
                  ...
                  <button
                    ...
                    type="button"
                  >
                    <i className={`ti ${verConfirmar ? "ti-eye-off" : "ti-eye"}`} />
                  </button>
                </div>
                {passConfirmar.length > 0 && passNueva !== passConfirmar && (
                  <p className="perfil-err-inline">Las contraseñas no coinciden</p>
                )}
                {passConfirmar.length > 0 && passNueva === passConfirmar && (
